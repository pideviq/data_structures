import unittest
from linkedlist.doubly_linked_list import Node, LinkedList
...
class TestDoublyLinkedList(unittest.TestCase):
    ...
    def test_linked_list_init_with_items(self):
        ...
        # Check head and tail
        with self.subTest(msg='Check head and tail', linked_list=linked_list,
                items=items):
            self.assertEqual(linked_list.head.data, items[0])
            self.assertEqual(linked_list.tail.data, items[-1])
        # Check first element
        with self.subTest(
    ...
    def test_linked_list_repr(self):
        ...
        items = list(self.typed_items.values())
        linked_list = LinkedList(items[:])
        string_repr = 'doubly_linked_list(['\
    ...
    def test_append(self):
        ...
        with self.subTest(
            ...
            self.assertEqual(linked_list.tail, second_node)
    ...
    def test_insert(self):
        ...
        with self.subTest(
            ...
            with self.assertRaises(IndexError):
                linked_list.insert(self.items_dict['insert_after_first'],
                                   self.items_dict['first'])
        linked_list.append(self.items_dict['first'])
        linked_list.append(self.items_dict['second'])
        ...
        self.assertEqual(first_node.data, self.items_dict['first'])
        second_node = first_node.next
        self.assertEqual(second_node.data,
                         self.items_dict['insert_after_first'])
        ...
        # Test insert after last element (append case)
        with self.subTest(
                msg='Test insert after last element', linked_list=linked_list,
                third_node=third_node):
            new_last = 'last'
            linked_list.insert(new_last, linked_list.tail.data)
            self.assertEqual(linked_list.tail.data, new_last)
            self.assertEqual(third_node.next, linked_list.tail)
    ...
    def test_remove(self):
        ...
        with self.subTest(
            ...
            self.assertEqual(new_third_node.data, '45d')
            self.assertEqual(new_third_node.next.data, None)
            self.assertEqual(new_third_node.prev.data, 'abc')
        with self.subTest(msg='Test remove first', linked_list=linked_list):
            ...
            self.assertIsNone(new_first_node.prev)
        with self.subTest(msg='Test remove last', linked_list=linked_list):
            linked_list.remove(linked_list.tail.data)
            new_last = linked_list.tail
            self.assertEqual(new_last.data, '45d')
            self.assertIsNone(new_last.next)
